...
public class DodgeBall implements Runnable{
	...
	private void createBall(int index){
		...
		// add newBall to the sequence.
		balls.addAfter(newBall);
	}
	
	private boolean validLocation(Ball newBall){
		/* Implement this pseudo-code:
		for every Ball b in balls {
			if (b.isColliding(newBall)) return false;
		}
		*/
		
		for (balls.start(); balls.isCurrent(); balls.advance()) {
			if(balls.getCurrent().isColliding(newBall)) return false;
		}
		return true;
	}
	...
	private void checkCollisions(){
		/* Implement using Sequence:
		// NB: You can't use nested loops on the same sequence
		for every Ball a in balls {
		    for every Ball b in balls, except a {
		...
		*/
		...
		BallSeq cloneB = balls.clone();
		
		for(balls.start(); balls.isCurrent(); balls.advance()) {
			for(cloneB.start(); cloneB.isCurrent(); cloneB.advance()) {
				
				if (balls.getCurrent().isColliding(cloneB.getCurrent())) {
					
					if (balls.getCurrent() != cloneB.getCurrent()) {
						balls.getCurrent().bounce(cloneB.getCurrent());
					}
				}
			}
			
			balls.getCurrent().bounceWalls(DodgeBall.BOUNDS);
		}
	...
	private void checkGameOver(){
		/* Implement this pseudo-code
		for every Ball b in balls {
			if player collides with b {
				remember that deathBall is b;
				call gameover();
			}
		}
        */
		...
		for(balls.start(); balls.isCurrent(); balls.advance()) {
			deathBall = balls.getCurrent();
			if (player.isColliding(deathBall)) {
				gameover();
			}
		}
	}
	...
	private class DodgeBallPanel extends JPanel{
		...
		public void paintComponent(Graphics g) 
		{
			...
			//draw game entities
			// TODO: draw every ball in balls
			player.draw(g);
			for (balls.start(); balls.isCurrent(); balls.advance()) {
				balls.getCurrent().draw(g);
			}
			...
			if (suspended){
				...
				if (alive){
					...
					g.fillOval(deathBall.getLoc().getX()-50, deathBall.getLoc().getY()-50, 100, 100);
					deathBall.draw(g);
					player.draw(g);
					g.setColor(Color.WHITE);
