...
public class BST {
	...
	private static void doFindInRange(Node n, String lo, String hi, Set<String> results) {
		...
		if (lo != null && lo.compareTo(n.key) >= 0) {
			...
			// Add this node's data to the set,
			// and then recurse to both sides.
			results.add(n.key);
			doFindInRange(n.left, lo, hi, results);
			doFindInRange(n.right, lo, hi, results);
		}
	...
	private static int doHeight(Node n) {
		if (n == null) {
			...
			return 1 + Math.max(doHeight(n.left), doHeight(n.right));
		}
	}
	...
	// TASK 2: CHECK TREE RANGE
	
	private Set<String> range() {
		Set<String> result = new HashSet<String>();
		// Call helper method with initial arguments
		// (the root, an unbounded range, and the set to drop tests into)
		doTreeRange(root, null, null, result);
		return result;
	}
	
	private static void doTreeRange(Node n, String lo, String hi, Set<String> results) {
		// we drop in a test and recurse to both sides.  No need
		// for cases, since we are just collecting constraints.
		// But: the range needs to be tightened in recursive calls.
	    if (n == null) {
	        return;
	    }
	    String keyRange = n.key + "<-(" + lo + "," + hi + ")";
	    
	    results.add(keyRange);
	    
	    doTreeRange(n.left, lo, n.key, results);
	    doTreeRange(n.right, n.key, hi, results);
	}
